...
.slider-area .single-slider {
	background-image: url('assets/img/hero/GS25header.jpg');
	background-position: center center; /* 가로, 세로 중앙 정렬 */
}
/* "내 위치 찾기" 버튼 스타일 */
#locationButton {
	display: block;
...
<body>
			...
			<div class="container">
				...
				<div class="row justify-content-center">
					<div class="col-xl-7 col-lg-8 col-md-10">
						<div class="section-tittle mb-70 text-center">
							<h2>지금 나에게 필요한건?</h2>
							<p>GS25의 다양한 상품들을 소개합니다!</p>
							<div class="search-box">
								<input id="searchKeyword" type="text"
									placeholder="GS25 상품을 검색해보세요!"> <i
									onclick="searchGSProductName()">🔍</i>
							</div>
						</div>
					</div>
				</div>
						...
						<nav>
							<div class="nav nav-tabs" id="nav-tab" role="tablist">
								<a class="nav-item nav-link active" id="nav-home-tab"
									...
									onclick="setGSProductOrderCondition('ORDERPOPULAR')">인기순</a> <a
									class="nav-item nav-link" id="nav-profile-tab"
									data-toggle="tab" href="javascript:void(0);" role="tab"
									aria-controls="nav-profile" aria-selected="false"
									onclick="setGSProductOrderCondition('ORDERHIGHPRICES')">가격
									높은순</a> <a class="nav-item nav-link" id="nav-contact-tab"
									data-toggle="tab" href="javascript:void(0);" role="tab"
									aria-controls="nav-contact" aria-selected="false"
									onclick="setGSProductOrderCondition('ORDERLOWPRICES')">가격
									낮은순</a>
							</div>
						</nav>
						<!--End Nav Button  -->
					...
					<div class="select-this">
						<div class="category-container">
							<button class="category dark"
								onclick="setGSProductCategory('ALLPRODUCT')">전체</button>
							<button class="category purple"
								onclick="setGSProductCategory('DAILYSUPPLIESPRODUCT')">생활용품</button>
							<button class="category green"
								onclick="setGSProductCategory('FOODPRODUCT')">식품</button>
							<button class="category blue"
								onclick="setGSProductCategory('BEVERAGEPRODUCT')">음료</button>
						</div>
					</div>
				</div>
				<!-- Nav Card -->
				<div class="tab-content" id="nav-tabContent">
					...
					<div class="tab-pane fade show active" id="nav-home"
						role="tabpanel" aria-labelledby="nav-home-tab">
						<div class="row" id="GSWrapper"></div>
					</div>
				</div>
				<!-- 페이지네이션 Start -->
				<nav class="blog-pagination justify-content-center d-flex">
					<ul class="pagination" id="GSPageNation">
					</ul>
				</nav>
				<!-- 페이지네이션 End -->
				<br>
						...
						<div class="section-tittle mb-70 text-center">
							...
							<div class="search-box">
								<input id="searchStoreKeyword" type="text"
									placeholder="주변에 GS25가 있는지 검색해보세요!"> <i
	...
	<!-- 사용자 정의 js -->
	<script src="assets/js/customization/GSProduct.js"></script>
    ...
    function getLocation() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                searchNearbyGS(pos); // 주변 300m 반경 내 편의점 검색
            }, function() {
 ...
 // 주변 300m 반경 내 GS 편의점 검색
    function searchNearbyGS(location) {
        const ps = new kakao.maps.services.Places();
        ps.categorySearch('CS2', function(result, status) { // 'CS2'는 편의점 카테고리
            if (status === kakao.maps.services.Status.OK) {
                result.forEach(function(place) {
                    if (place.place_name.includes("GS")) { // 이름에 "CU"가 포함된 편의점만 처리
                        displayMarker(place);
                    }
                });

                if (!result.some(place => place.place_name.includes("GS"))) {
                    alert('주변에 GS 편의점을 찾을 수 없습니다.');
                }
            } else {
