...
const kakaoPayApproveUrl = "/honeyComboFactory/purchase/kakao/approval";
...
const requestKakaoPayApprove = (pg_token) => {
	...
	$.ajax({
		...
		success: (response) => {
			...
			if (response && response.approved_at) {
				Swal.fire({
					icon: "success",
					title: "결제가 완료되었습니다."
				}).then(() => {
				...
				console.log("결제 실패");
				// 스위트 알럿창 출력
				printSweetAlert("error", "카카오 결제 승인이 실패했습니다.", "지속될 시 관리자에게 문의하세요.", "error.do");
			}
		},
