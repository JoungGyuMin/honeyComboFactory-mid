...
const loadMoreHotUrl = "/honeyComboFactory/GsHotIssueProductServlet"; // 핫이슈 불러오기 서블릿 url
const loadMorePlusUrl = "/honeyComboFactory/GsPlusOneProductServlet"; // +1증정 불러오기 서블릿 url
const loadMoreGSUrl = "/honeyComboFactory/OrderGsProductServlet"; // GS상품 불러오기 서블릿 url
const searchGSProductNameUrl = "/honeyComboFactory/SearchGsProductServlet"; // GS상품 이름 검색 서블릿 url
...
$(document).ready(function() {
	...
	// GS 상품 페이지네이션 이벤트 등록
	$(document).on("click", ".GS", function(event) { // 페이지네이션 클릭 시
		event.preventDefault(); // 기본 이벤트 방지

		changePage("GS", $(this)); // 페이지 이동
	});
...
// 상품 카테고리 변경 기능
const setGSProductCategory = (category) => {
...
// GS상품 정렬 기능
const setGSProductOrderCondition = (orderCondition) => {
	...
	console.log("정렬 시 현재 검색된 GS 상품 정렬 조건 : [" + productState.orderCondition + "]");
...
// GS상품 이름 검색 기능
const searchGSProductName = () => {
	console.log("GS상품 이름 검색 실행");
	productState.searchKeyword = $("#searchKeyword").val();
	console.log("검색한 GS상품 이름 : [" + productState.searchKeyword + "]");
	...
	$.ajax({ // 비동기
		url: searchGSProductNameUrl, // 보낼 주소
		type: 'GET', // 방식
		data: { // 보낼 값
			searchKeyword: productState.searchKeyword,
			GSProductPageNumber: productState.pageNumber["GS"],
			GSProductContentCount: productState.contentCount["GS"],
			GSProductOrderCondition: productState.orderCondition
		},
		dataType: 'json', // 받을 타입
		success: (response) => { // 성공적이라면
			console.log("받은 검색 GS상품 데이터 : [" + response + "]"); // 로그 찍기
			// 카테고리는 전부로 설정
...
const loadDatas = (type) => {
	...
	if (type === "hotIssue") { // 핫이슈 상품이라면
		$.ajax({ // 비동기
			...
			type: 'GET', // 방식
			data: { // 보낼 값
				hotGSProductPageNumber: productState.pageNumber[type],
				hotGSProductContentCount: productState.contentCount[type]
			},
			dataType: 'json', // 받을 타입
			success: (response) => { // 성공적이라면
		...
		$.ajax({ // 비동기
			...
			type: 'GET', // 방식
			data: { // 보낼 값
				plusGSProductPageNumber: productState.pageNumber[type],
				plusGSProductContentCount: productState.contentCount[type]
			},
			dataType: 'json', // 받을 타입
			success: (response) => { // 성공적이라면
	...
	else if (type === "GS") { // CU 상품이라면
		$.ajax({ // 비동기
			url: loadMoreGSUrl, // 보낼 주소
			type: 'GET', // 방식
			data: { // 보낼 값
				GSProductPageNumber: productState.pageNumber[type],
				GSProductContentCount: productState.contentCount[type],
				GSProductOrderCondition: productState.orderCondition,
				GSProductCategory: productState.category
			},
			dataType: 'json', // 받을 타입
			success: (response) => { // 성공적이라면
				console.log("받은 GS 상품 데이터 : [" + response + "]"); // 로그 찍기
				...
				<div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
					<div class="single-popular-items mb-50 text-center">
						...
						<div class="popular-caption">
							...
							<span>`+ productData.productSinglePrice + `원</span>
						</div>
					</div>
				</div>
			`);
...
function makePageNation(type, totalNumber) {
	...
	let group = Math.ceil(productState.pageNumber[type] / productState.pageGroupSize); // 현재 그룹
