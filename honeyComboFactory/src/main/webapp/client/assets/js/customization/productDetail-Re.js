 ...
 *  상품 상세 js
 */
const loadMoreReviewUrl = "/honeyComboFactory/ProductDetailReviewServlet"; // 리뷰 불러오기 서블릿 url
const insertCartUrl = "/honeyComboFactory/AddCartProductServlet"; // 장바구니 상품 담기 서블릿 url
const insertReviewUrl = "/honeyComboFactory/ProductDetailInsertReviewServlet"; // 리뷰 등록 서블릿 url
const updateReviewUrl = "/honeyComboFactory/ProductDetailUpdateReviewServlet"; // 리뷰 수정 서블릿 url
const deleteReviewUrl = "/honeyComboFactory/ProductDetailDeleteReviewServlet"; // 리뷰 삭제 서블릿 url
...
let loginedMemberNumber = 0; // 로그인 회원 번호
...
const insertReview = () => {
	...
	$.ajax({
		...
		dataType: "json", // 받을 타입
		success: (response) => { // 성공 시 처리
			console.log("받은 리뷰등록 데이터 : [" + response + "]");
			if (response.rivewData === 0) { // 중복 등록 시
				console.log("리뷰 중복 등록 시도");
				alert("이미 작성된 리뷰가 있습니다.");
			} else if (response.rivewData === 1) { // 등록 실패 시
...
const updateReview = (reviewNumber) => {
	...
	if (updateReviewBtn.text() === "수정") { // 수정 버튼 클릭 시
		...
		// 별점 박스 완전 교체 (초기화 꼼수)
		$("#rating-reply-" + reviewNumber).replaceWith('<div id="rating-reply-' + reviewNumber + '"></div>');
	...
	$.ajax({
		...
		success: (response) => { // 성공 시 처리
			if (response === "true") { // 삭제 완료 시
				...
				// 수정한 리뷰의 readonly 속성 설정
				$("#reviewComment-" + reviewNumber).prop("readonly", true);
				// 읽기 전용 css 추가
				$("#reviewComment-" + reviewNumber).addClass("readonlyReview");
				$('#rating-reply-' + reviewNumber).starRating('setOptions', {
					readOnly: true,    // 별점 수정 가능
					disableAfterRate: true // 별점 클릭 후에도 수정 가능
				});
			} else { // 삭제 실패 시
